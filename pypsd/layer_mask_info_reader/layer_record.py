...
def read_pascal_string_pad4(buf: BinaryIO):
...
def read_extra_data(buf: BinaryIO):
    read_layer_mask_data(buf)
    read_layer_blending_ranges_data(buf)
    layer_name = read_pascal_string_pad4(buf)
    # TODO: read "Additional Layer Information" structure
    additional_layer_info_data = buf.read()
    return ExtraLayerData(layer_name)


def read_layer_record(buf: BinaryIO):
    top = read_int32(buf)
    left = read_int32(buf)
    ...
    extra_data = read_extra_data(BytesIO(extra_data_bytes))
